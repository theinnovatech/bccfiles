<template>
    <div class="space-y-6">
        <div class="shadcn-card p-6">
            ...
            <form class="grid gap-4 md:grid-cols-2" @submit.prevent="loadRequest">
                <div>
                    <label class="mb-1 block text-sm font-medium">Approved Request</label>
                    <Select
                        v-model="requestId"
                ...
                <div>
                    <label class="mb-1 block text-sm font-medium">Received By</label>
                    <Select
                        v-model="receivedBy"
                ...
                <div class="md:col-span-2">
                    <Button type="button" label="Load Request" @click="loadRequest" />
                </div>
            </form>
        </div>
        ...
        <div v-if="selectedRequest" class="shadcn-card p-4 sm:p-6">
            <p class="mb-4 text-sm text-[#5b7fbf]">
                {{ selectedRequest.request_number }} • {{ selectedRequest.department?.name }}
            ...
            <div class="space-y-4">
                ...
                >
                    <div>
                        <p class="font-medium text-[#002a7a]">{{ line.display_name }}</p>
                        <p v-if="line.property_number" class="text-sm text-[#5b7fbf]">Property No.: {{ line.property_number }}</p>
                        <p class="text-sm text-[#5b7fbf]">Remaining: {{ line.remaining }}</p>
                    </div>
                    <div class="grid gap-3 md:grid-cols-[minmax(0,1fr)_140px] md:items-end">
                        <div class="min-w-0">
                            <label class="mb-1 block text-sm font-medium">{{ isEquipmentRequest ? 'Barcode / Property No.' : 'Barcode' }}</label>
                            <input
                                type="text"
                        ...
                        <div class="min-w-0">
                            <label class="mb-1 block text-sm font-medium">Issue Qty</label>
                            <InputNumber
            ...
            <div class="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center">
                <Button class="w-full sm:w-auto" label="Process Issuance" :loading="loading" @click="submit" />
                <UiButton class="w-full sm:w-auto" variant="outline" :disabled="loading" @click="cancel">Cancel</UiButton>
            </div>
        </div>

        <UiCard title="Issuance History" description="Recent issuances processed from approved supply requests.">
            <TableFilters
                v-model="filters"
            ...
            <div class="obims-table-wrap">
                ...
                >
                    ...
                    <Column header="Request No.">
                        <template #body="{ data }">{{ data.request?.request_number }}</template>
                    </Column>
                    <Column header="Department">
                        <template #body="{ data }">{{ data.request?.department?.name }}</template>
                    </Column>
                    <Column header="Issued By">
                        <template #body="{ data }">{{ data.issuer?.name }}</template>
                    </Column>
                    <Column header="Received By">
                        <template #body="{ data }">{{ data.receiver?.name }}</template>
                    </Column>
                    <Column header="Issued">
                        <template #body="{ data }">
                            <span class="text-sm text-[#002a7a]">{{ formatIssuedLines(data) }}</span>
                        </template>
                    </Column>
...
const { filters, filteredItems: filteredIssuances, hasActiveFilters, resetFilters } = useTableFilters(issuances, filterConfig);

const isEquipmentRequest = computed(() => selectedRequest.value?.request_type === 'equipments');
...
function formatIssuedLines(issuance) {
    return (issuance.details ?? [])
        .map((detail) => {
            const name = detail.equipment?.name ?? detail.item?.item_name ?? 'Line item';
...
function totalQuantity(issuance) {
    return (issuance.details ?? []).reduce((sum, detail) => sum + detail.quantity, 0);
}

async function loadApprovedRequests() {
    const { data } = await api.get('/requests/list');
    const rows = (data.data ?? data).filter((row) => ['approved', 'partially_issued'].includes(row.status));
...
async function loadRequest() {
    ...
    issueLines.value = data.details
        .filter((detail) => detail.quantity_requested - detail.quantity_issued > 0)
        .map((detail) => {
        const remaining = detail.quantity_requested - detail.quantity_issued;
        ...
        if (equipmentRequest) {
            return {
                ...
                barcode: detail.equipment?.barcode || detail.equipment?.property_number || '',
                remaining,
...
async function loadIssuances() {
    ...
    try {
        ...
        notify.error(error.response?.data?.message || 'Unable to load issuance history.');
    } finally {
...
async function submit() {
    ...
    try {
        ...
        notify.success('Issuance was processed successfully.', 'Issuance completed');
        selectedRequest.value = null;
        issueLines.value = [];
        requestId.value = null;
        receivedBy.value = null;
        ...
        notify.error(error.response?.data?.message || error.message || 'Unable to process issuance.', 'Issuance failed');
    } finally {
        loading.value = false;
    }
}
