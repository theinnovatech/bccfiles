<template>
    <div class="space-y-6">
        ...
        <div
            v-if="loading"
            class="shadcn-card flex min-h-[240px] items-center justify-center p-6"
        >
            <div class="text-center">
                <svg
                    class="mx-auto h-8 w-8 animate-spin text-[#0038a8]"
                    viewBox="0 0 24 24"
                    fill="none"
                >
                    <circle
        ...
        <div v-else-if="error" class="shadcn-card p-6">
            ...
            <UiButton class="mt-4" variant="outline" @click="goBack"
                >Return to Supply Requests</UiButton
            >
        </div>
        ...
        <template v-else-if="request">
            <div class="shadcn-card p-4 sm:p-6">
                ...
                >
                    <div class="min-w-0">
                        ...
                        <div class="mt-3 flex flex-wrap items-center gap-2">
                            <UiBadge :variant="statusVariant(request.status)">{{
                ...
                <p v-if="request.remarks" class="mt-4 text-sm text-[#5b7fbf]">
                    {{ request.remarks }}
                </p>
            </div>
...
const isEquipmentRequest = computed(
    () => request.value?.request_type === "equipments",
);
const detailsTitle = computed(() =>
    isEquipmentRequest.value ? "Requested Equipments" : "Requested Items",
);
...
const detailsColumnHeader = computed(() =>
    isEquipmentRequest.value ? "Equipment" : "Item",
);
...
async function load() {
    ...
    try {
        ...
        error.value =
            ...
            "Unable to load this supply request.";
        request.value = null;
    } finally {
        loading.value = false;
    }
...
async function approve() {
    ...
    try {
        ...
        notify.error(
            err.response?.data?.message || "Unable to approve this request.",
        );
    } finally {
        acting.value = false;
    }
...
async function reject() {
    acting.value = true;
    try {
        await api.post(`/requests/${props.requestId}/reject`, {
        ...
        notify.error(
            err.response?.data?.message || "Unable to reject this request.",
        );
    } finally {
        acting.value = false;
    }
