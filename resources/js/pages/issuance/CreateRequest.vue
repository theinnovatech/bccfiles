        ...
        <div class="shadcn-card overflow-hidden">
            <div class="stock-op-hero border-b border-[#c8d6ef] px-4 py-5 sm:px-6">
                <div class="flex flex-wrap items-start justify-between gap-4">
                    <div>
                        <h3 class="text-lg font-semibold text-[#002a7a]">Create Supply Request</h3>
                        <p class="mt-1 text-sm text-[#5b7fbf]">
                            Submit a request for supplies or equipment from your department.
                        </p>
                    </div>
                ...
                <div class="mt-4 grid gap-2 sm:grid-cols-2">
                    <button
                        ...
                        :class="{ 'stock-op-tab-card-active': form.request_type === tab.key }"
            ...
            <form class="request-form-content min-w-0 space-y-5 p-4 sm:p-6" @submit.prevent="submit">
                <div class="grid gap-4 md:grid-cols-2">
                    <div>
                        <label class="mb-1 block text-sm font-medium text-[#002a7a]">Department</label>
                        <Select
                    ...
                    <div>
                        <label class="mb-1 block text-sm font-medium text-[#002a7a]">Request Type</label>
                        <div class="flex h-[42px] items-center rounded-md border border-[#c8d6ef] bg-[#eef2fa] px-3 text-sm font-medium text-[#0038a8]">
                            {{ form.request_type === 'items' ? 'Items / Supplies' : 'Equipments' }}
                        </div>
                    </div>
                </div>
                ...
                <div class="request-lines-panel rounded-xl border border-[#c8d6ef] bg-[#f8fafc] p-4">
                    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
                        <div>
                            <h4 class="font-semibold text-[#002a7a]">
                                {{ form.request_type === 'items' ? 'Requested Items' : 'Requested Equipments' }}
                            </h4>
                            <p class="mt-0.5 text-sm text-[#5b7fbf]">
                                Add one or more {{ form.request_type === 'items' ? 'items' : 'equipments' }} to include in this request.
                            </p>
                        </div>
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                    ...
                    <UiButton type="button" variant="outline" :disabled="loading" @click="goBack">
                        Cancel
                    </UiButton>
                </div>
            </form>
...
const IconEquipments = {
    render() {
        return h('svg', { fill: 'none', viewBox: '0 0 24 24', stroke: 'currentColor', 'stroke-width': '2' }, [
            h('path', { 'stroke-linecap': 'round', 'stroke-linejoin': 'round', d: 'M9 3.75H6.912a2.25 2.25 0 00-2.15 1.588L2.35 13.177a2.25 2.25 0 00-.1.661V18a2.25 2.25 0 002.25 2.25h15A2.25 2.25 0 0021.75 18v-4.162c0-.224-.034-.447-.1-.661L19.24 5.338a2.25 2.25 0 00-2.15-1.588H15M9 3.75h6M9 3.75v1.5m6-1.5v1.5m-9 4.5h10.5' }),
        ]);
    },
};
...
async function submit() {
    ...
    try {
        ...
        notify.success('Your supply request was submitted successfully.', 'Request submitted');
        setTimeout(() => {
            window.location.href = `/requests/${data.id}`;
        }, 600);
    } catch (error) {
        notify.error(error.response?.data?.message || 'Unable to submit supply request.', 'Submit failed');
    } finally {
        loading.value = false;
    }
}
