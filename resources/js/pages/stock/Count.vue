                ...
                >
                    <div>
                        ...
                        <p class="mt-1 text-sm text-[#5b7fbf]">
                            Count items on hand, record physical quantities, and
                            auto-apply adjustments for variances.
                        </p>
                        ...
                        >
                            ...
                            >
                            <span
                                class="stock-count-badge stock-count-badge-muted"
                            >
                        ...
                        >
                            ...
                            >
                                <path
                                    stroke-linecap="round"
                                    stroke-linejoin="round"
                                    d="M5.25 5.653v15.694a2.25 2.25 0 01-2.25 2.25h-1.5a2.25 2.25 0 01-2.25-2.25V5.653zM16.5 3.75h-1.5a2.25 2.25 0 00-2.25 2.25v16.5a2.25 2.25 0 002.25 2.25h1.5a2.25 2.25 0 002.25-2.25V6a2.25 2.25 0 00-2.25-2.25z"
                                />
                            </svg>
                            Start Count Session
                        </UiButton>
            ...
            <div class="stock-op-content p-4 sm:p-6">
                ...
                <div v-else-if="!session" class="stock-op-empty">
                    ...
                    <p class="mt-1 text-xs text-[#5b7fbf]">
                        Start a session to scan items and record physical
                        quantities.
                    </p>
                    <UiButton
                        class="mt-4"
                ...
                <template v-else>
                    <div v-if="isInProgress" class="space-y-6">
                            ...
                            <div class="stock-op-form-panel">
                                <div class="stock-op-form-header">
                                    ...
                                    <p class="stock-op-form-desc">
                                        Enter the actual quantity on hand for
                                        this item.
                                    </p>
                                </div>
                                ...
                                >
                                    ...
                                    <div
                                        class="stock-op-highlight"
                                        :class="varianceClass"
                                    >
                                        <span class="stock-op-highlight-label"
                                            >Variance vs system</span
                                        >
                                        <strong
                                            class="stock-op-highlight-value"
                                            >{{ variancePreview }}</strong
                                        >
                                    </div>
                                    <div
                        ...
                        <div v-else class="stock-op-empty">
                            ...
                            <p class="mt-1 text-xs text-[#5b7fbf]">
                                Scan an item barcode to record its physical
                                count.
                            </p>
                        </div>
                    </div>
                    ...
                    <div v-else class="stock-op-alert stock-op-alert-success">
                        ...
                        <div>
                            <p class="stock-op-alert-title">
                                Session completed
                            </p>
                            <p class="stock-op-alert-text">
                                This count session is closed. Use View Records
                                to review results or start a new session.
                            </p>
                        </div>
                    </div>
                </template>
...
async function loadActiveSession() {
    ...
    try {
        ...
        const { data: fullSession } = await api.get(
            `/stock-count/sessions/${active.id}`,
        );
        session.value = fullSession;
    } catch (error) {
        notify.error(
            error.response?.data?.message || "Unable to load count session.",
        );
    } finally {
...
async function startSession() {
    ...
    try {
        ...
        notify.error(
            error.response?.data?.message || "Unable to start session.",
        );
    } finally {
...
async function recordCount() {
    ...
    try {
        const { data } = await api.post(
            `/stock-count/sessions/${session.value.id}/items`,
            {
        ...
        const index = session.value.items.findIndex(
            (row) => row.id === data.count_item.id,
        );
        ...
        notify.error(
            error.response?.data?.message || "Unable to record count.",
        );
    } finally {
...
async function completeSession() {
    ...
    try {
        const { data } = await api.post(
            `/stock-count/sessions/${session.value.id}/complete`,
        );
        session.value = data;
        clearSelection();
        ...
        notify.error(
            error.response?.data?.message || "Unable to complete session.",
        );
    } finally {
