<template>
    <div class="space-y-6">
        ...
        <div v-if="existingEquipment" class="stock-op-alert stock-op-alert-warn">
            ...
            <div>
                ...
                <p class="stock-op-alert-text">
                    {{ existingEquipment.name }} · {{ existingEquipment.property_number }}
                </p>
            </div>
        </div>
        ...
        <div v-if="showForm" class="stock-op-form-panel">
            <div class="stock-op-form-header">
                <h4 class="stock-op-form-title">{{ editingId ? 'Edit equipment' : 'Register equipment' }}</h4>
                <p class="stock-op-form-desc">Record equipment details separate from inventory items.</p>
            </div>

            <form class="grid gap-4 md:grid-cols-2" @submit.prevent="save">
                <div v-if="editingId">
                    <label class="mb-1 block text-sm font-medium text-[#002a7a]">Property No.</label>
                    <InputText v-model="form.property_number" class="w-full" readonly />
                </div>
                <div v-else class="md:col-span-2">
                    <p class="rounded-md border border-[#c8d6ef] bg-[#eef2fa] px-3 py-2 text-xs text-[#5b7fbf]">
                        Property number will be generated automatically when you save this equipment.
                    </p>
                </div>

                <div v-if="form.barcode">
                    <label class="mb-1 block text-sm font-medium text-[#002a7a]">Barcode</label>
                    <InputText v-model="form.barcode" class="w-full" readonly />
                </div>
                <div v-else-if="noBarcodeMode || editingId" class="md:col-span-2">
                    <p class="rounded-md border border-dashed border-[#c8d6ef] bg-white px-3 py-2 text-xs text-[#5b7fbf]">
                        No barcode assigned for this equipment.
                    </p>
                </div>

                <div>
                    <label class="mb-1 block text-sm font-medium text-[#002a7a]">Name</label>
                    <InputText v-model="form.name" class="w-full" required />
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-[#002a7a]">Category</label>
                    <Select
                        ...
                        required
                    />
                    <p v-if="!categories.length" class="mt-1 text-xs text-[#5b7fbf]">
                        Add categories first in Master Data → Equipment Categories.
                    </p>
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-[#002a7a]">Type</label>
                    <InputText v-model="form.type" class="w-full" required />
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-[#002a7a]">Qty</label>
                    <InputNumber v-model="form.quantity" class="w-full" :min="1" required />
                </div>
                <div class="md:col-span-2">
                    <label class="mb-1 block text-sm font-medium text-[#002a7a]">Description</label>
                    <Textarea v-model="form.description" class="w-full" rows="3" placeholder="Brief description of the equipment" />
                </div>
                <div class="md:col-span-2">
                    <label class="mb-1 block text-sm font-medium text-[#002a7a]">Specs</label>
                    <Textarea v-model="form.specs" class="w-full" rows="3" placeholder="Specifications, model, serial number, etc." />
                </div>
                <div class="md:col-span-2 flex flex-col gap-3 sm:flex-row">
                    ...
                    <UiButton v-if="editingId" type="button" variant="outline" @click="resetForm">Cancel edit</UiButton>
                    <UiButton v-else type="button" variant="outline" @click="resetForm">Clear form</UiButton>
                </div>
            </form>
        </div>

        <div v-else-if="!editingId && !existingEquipment" class="stock-op-empty">
            ...
            <p class="mt-3 text-sm font-medium text-[#002a7a]">No barcode scanned yet</p>
            <p class="mt-1 text-xs text-[#5b7fbf]">Scan a barcode above or continue without one.</p>
        </div>
    </div>
</template>
...
async function lookupBarcode(code) {
    ...
    try {
        const { data } = await api.get(`/equipments/barcode/${encodeURIComponent(code)}`);
        ...
        notify.error(error.response?.data?.message || 'Unable to look up barcode.');
    }
}
...
async function loadCategories() {
    try {
        ...
        categories.value = data.filter((category) => category.is_active !== false);
    } catch {
...
async function save() {
    ...
    try {
        ...
        if (editingId.value) {
            ...
            notify.success(`Equipment saved with property no. ${data.property_number}.`);
        }

        resetForm();
    } catch (error) {
        notify.error(error.response?.data?.message || 'Unable to save equipment.');
    } finally {
        saving.value = false;
    }
}
