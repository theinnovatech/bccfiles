<template>
    <div class="space-y-6">
        ...
        <UiCard title="Counted Items" description="Review physical count records by session.">
            <div class="mb-4 grid gap-4 md:grid-cols-[minmax(0,1fr)_auto] md:items-end">
                <div>
                    <label class="mb-1 block text-sm font-medium text-[#002a7a]">Count session</label>
                    <Select
                ...
                <div v-if="session" class="flex flex-wrap items-center gap-2">
                    ...
                    <span class="stock-count-badge stock-count-badge-muted">
                        {{ countedItems.length }} item{{ countedItems.length === 1 ? '' : 's' }}
                    </span>
            ...
            <div v-else-if="!session" class="stock-op-empty">
                ...
                <p class="mt-3 text-sm font-medium text-[#002a7a]">No count sessions found</p>
                <p class="mt-1 text-xs text-[#5b7fbf]">Start a physical inventory session to record counts.</p>
            </div>
            ...
            <div v-else-if="!countedItems.length" class="stock-op-empty">
                ...
                <p class="mt-3 text-sm font-medium text-[#002a7a]">No items counted yet</p>
                <p class="mt-1 text-xs text-[#5b7fbf]">This session has no recorded counts.</p>
            </div>
            ...
            <div v-else class="obims-table-wrap">
                ...
                >
                    ...
                    <Column header="Variance">
                        <template #body="{ data }">
                            <span class="stock-count-variance" :class="varianceRowClass(data.variance)">
                                {{ data.variance }}
                            </span>
                        </template>
                    </Column>
                    <Column header="Adjusted">
                        <template #body="{ data }">
                            <span
                                ...
                                :class="data.adjustment_created ? 'stock-count-badge-success' : 'stock-count-badge-muted'"
                            >
...
const statusLabel = computed(() => (isInProgress.value ? 'In progress' : 'Completed'));

const statusBadgeClass = computed(() =>
    isInProgress.value ? 'stock-count-badge-active' : 'stock-count-badge-complete',
);
...
const { filters, filteredItems, hasActiveFilters, resetFilters } = useTableFilters(countedItems, filterConfig);
...
async function loadSessions(preferredId = null) {
    ...
    try {
        ...
        const queryId = preferredId ?? Number(new URLSearchParams(window.location.search).get('session'));
        const preferred = sessions.value.find((row) => row.id === queryId);
        const inProgress = sessions.value.find((row) => row.status === 'in_progress');

        selectedSessionId.value = preferred?.id ?? inProgress?.id ?? sessions.value[0].id;
        await loadSession();
    } catch (error) {
        notify.error(error.response?.data?.message || 'Unable to load count sessions.');
    } finally {
...
async function loadSession() {
    ...
    try {
        const { data } = await api.get(`/stock-count/sessions/${selectedSessionId.value}`);
        session.value = data;
        ...
        notify.error(error.response?.data?.message || 'Unable to load session records.');
        session.value = null;
    } finally {
