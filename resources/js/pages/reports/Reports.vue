        ...
        <UiCard>
            <template #header>
                <div>
                    ...
                    <p class="shadcn-card-description">Select a report type to preview and download as PDF.</p>
                </div>
            </template>
            ...
            <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                <button
                    ...
                    :class="{ 'report-type-btn-active': activeType === report.type }"
        ...
        <UiCard>
            <template #header>
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        <h3 class="shadcn-card-title">{{ activeLabel || 'Report Preview' }}</h3>
                        <p class="shadcn-card-description">
                            <span v-if="!activeType">Choose a report above to preview it here.</span>
                            <span v-else-if="loading">Loading report data...</span>
                            <span v-else-if="reportRows.length">{{ reportRows.length }} record{{ reportRows.length === 1 ? '' : 's' }} found</span>
                            <span v-else>No records available for this report.</span>
                        </p>
                    </div>
                    <UiButton v-if="activeType && !loading && reportRows.length" class="w-full sm:w-auto" @click="exportPdf">
            ...
            <div v-if="!activeType" class="report-placeholder">
                ...
                <p class="mt-3 text-sm font-medium text-[#002a7a]">No report selected</p>
                <p class="mt-1 text-xs text-[#5b7fbf]">Pick a report type to preview its contents.</p>
            </div>
            ...
            <template v-else>
                ...
                <div class="mt-4">
                    <ReportPreview v-if="activeView === 'table'" :type="activeType" :data="reportRows" />
                    <ReportPdfViewer v-else :type="activeType" :has-data="reportRows.length > 0" />
                </div>
            </template>
        </UiCard>
    </div>
...
const activeLabel = computed(() => reports.find((report) => report.type === activeType.value)?.label);
...
async function loadReport(type) {
    ...
    try {
        ...
        reportRows.value = Array.isArray(data.data) ? data.data : (data.data ? [data.data] : []);
    } catch (error) {
        notify.error(error.response?.data?.message || 'Unable to load report preview.');
        reportRows.value = [];
    } finally {
        loading.value = false;
    }
