    ...
    <div :class="embedded ? 'space-y-6' : 'mx-auto max-w-3xl space-y-6'">
        ...
        <div v-if="existingItem" class="stock-op-alert stock-op-alert-warn">
            ...
            <div>
                ...
                <p class="stock-op-alert-text">
                    {{ existingItem.item_name }} · Current stock: {{ existingItem.current_stock }}
                </p>
            </div>
        </div>
        ...
        <div v-if="showForm" class="stock-op-form-panel">
            <div class="stock-op-form-header">
                ...
                <p class="stock-op-form-desc">Complete the form to add this item to inventory.</p>
            </div>
            ...
            <form class="grid gap-4 md:grid-cols-2" @submit.prevent="save">
                ...
                <div>
                    <label class="mb-1 block text-sm font-medium text-[#002a7a]">Item Name</label>
                    <InputText v-model="form.item_name" class="w-full" required />
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-[#002a7a]">Brand</label>
                    <InputText v-model="form.brand" class="w-full" />
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-[#002a7a]">Category</label>
                    <Select v-model="form.category_id" :options="categories" optionLabel="name" optionValue="id" class="w-full" />
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-[#002a7a]">Unit</label>
                    <Select v-model="form.unit_id" :options="units" optionLabel="name" optionValue="id" class="w-full" />
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-[#002a7a]">Storage Location</label>
                    <Select v-model="form.location_id" :options="locations" optionLabel="name" optionValue="id" class="w-full" />
                </div>
                <div>
                    <label class="mb-1 block text-sm font-medium text-[#002a7a]">Minimum Stock</label>
                    <InputNumber v-model="form.minimum_stock" class="w-full" :min="0" />
                </div>
                ...
                <div class="md:col-span-2 flex flex-col gap-3 sm:flex-row">
                    <UiButton type="submit" :loading="saving">Save Item</UiButton>
                    <UiButton type="button" variant="outline" @click="resetForm">Clear form</UiButton>
                </div>
            </form>
        </div>
        ...
        <div v-else-if="embedded && !existingItem" class="stock-op-empty">
            ...
            <p class="mt-1 text-xs text-[#5b7fbf]">Scan a new barcode above to register an item.</p>
        </div>
    </div>
</template>
...
async function save() {
    ...
    try {
        ...
        notify.error(error.response?.data?.message || 'Unable to register item.', 'Registration failed');
    } finally {
        saving.value = false;
    }
}
